...
export default function AddMaintenance() {
  ...
  if (fetching)
    return (
      <div
        className="h-screen flex items-center justify-center font-medium text-slate-400 bg-[#F8FAFC]"
        style={{ fontFamily: themeFont }}
      >
        <Loader2 className="animate-spin text-yellow-500 mr-2" />{" "}
        Synchronizing...
      </div>
    );
  ...
  return (
    <ThemeProvider theme={muiTheme}>
      <div
        className="p-4 animate-in fade-in duration-700"
        style={{ fontFamily: themeFont }}
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 font-medium italic text-sm">
                Vehicle service and expense history.
              </p>
            </div>
          </div>
          <button
            onClick={() => setShowModal(true)}
            className="bg-slate-900 text-yellow-400 px-8 py-4 rounded-2xl font-bold text-xs uppercase tracking-widest shadow-xl flex items-center gap-3 hover:bg-slate-800 transition-all active:scale-95"
          >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  <th className="p-6 text-[11px] font-semibold uppercase text-slate-400">
                    Date
                  </th>
                  <th className="p-6 text-[11px] font-semibold uppercase text-slate-400">
                    Vehicle
                  ...
                  <th className="p-6 text-[11px] font-semibold uppercase text-slate-400 text-right">
                    Amount (LKR)
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {logs.map((log) => (
                  ...
                  >
                    ...
                    <td className="p-6">
                      <span className="bg-slate-900 text-yellow-400 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase">
                        {log.vehicleNo}
                      </span>
                    </td>
                    <td className="p-6 text-sm text-slate-800 font-medium">
                      {log.description}
                    </td>
                    <td className="p-6 text-sm text-slate-700 italic font-medium">
                      {log.driver}
                    </td>
        ...
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-4xl max-h-[95vh] overflow-y-auto rounded-[2.5rem] shadow-2xl relative custom-scrollbar p-10 animate-in zoom-in-95 duration-300">
              ...
              <h2 className="text-2xl font-bold text-slate-900 uppercase mb-1">
                Add Service Record
              </h2>
              <p className="text-sm text-slate-500 mb-8 font-medium">
                Please enter maintenance details accurately.
              </p>
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField {...params} placeholder="Select Driver" />
                    )}
                  />
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Autocomplete
                    ...
                    onChange={(e, val) =>
                      setFormData({ ...formData, maintenanceType: val })
                    }
                    renderInput={(params) => (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    className="input-style-clean"
                    type="text"
                    value={formData.shopGarage}
                    onChange={(e) =>
                      setFormData({ ...formData, shopGarage: e.target.value })
                    }
                  />
                </div>
                ...
                <div className="md:col-span-2 flex gap-4 mt-4">
                  <button
                    type="button"
                    onClick={() => setShowModal(false)}
                    className="flex-1 py-4 border border-slate-200 rounded-xl font-bold uppercase text-[10px] tracking-widest text-slate-400 hover:bg-slate-50 transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-[2] py-4 bg-slate-900 text-yellow-400 rounded-xl font-bold uppercase text-[10px] tracking-widest shadow-xl hover:bg-slate-800 disabled:opacity-50 transition-all"
                  >
