...
import { 
  User, RefreshCcw, Truck, FileSpreadsheet, FileText
} from "lucide-react";
...
export default function DriverReport() {
  ...
  const exportToExcel = () => {
    const dataToExport = filteredData.map(t => ({
      "Tour ID": t.tourId, "Date": t.date, "Driver": t.driver,
      "Vehicle": t.vehicle, "From": t.fromLocation, "To": t.toLocation, "Dept": t.department
    }));
      ...
      <div className="p-4 space-y-6 animate-in fade-in duration-700" style={{ fontFamily: themeFont }}>
        ...
        <header className="flex flex-col md:flex-row justify-between items-center bg-white p-5 rounded-3xl border border-slate-100 shadow-sm gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800 tracking-tight leading-none">Driver Journey Log</h1>
              <p className="text-slate-500 font-medium italic mt-1 text-[10px]">Historical transport records.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white p-5 rounded-[2rem] shadow-sm border border-slate-50">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} placeholder="Search Driver..." />}
            />
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.length > 0 ? filteredData.map((t, i) => (
                  <tr key={i} className="hover:bg-yellow-50/30 transition-all group">
                    <td className="px-6 py-4">
                       <span className="bg-slate-900 text-yellow-400 px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider">{t.tourId}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-yellow-400 flex items-center justify-center text-slate-900"><User size={12}/></div>
                        <span className="text-sm font-medium text-slate-700 italic">{t.driver}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-slate-500">
                        <Truck size={14} />
                        <span className="text-[11px] font-bold uppercase">{t.vehicle}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-800">{t.fromLocation} → {t.toLocation}</td>
                    <td className="px-6 py-4 text-[10px] font-medium text-slate-400">{t.date} | {t.time}</td>
                  </tr>
                )) : (
                  <tr>
                    <td colSpan="5" className="p-20 text-center text-slate-300 font-bold italic uppercase tracking-widest text-xs">No records found.</td>
                  </tr>
                )}
              </tbody>
            </table>
