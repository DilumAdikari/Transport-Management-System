...
export default function Login({ setUser }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("http://192.168.1.10:5000/api/login", {
      ...
      setError(
        err.response?.data?.message || "Login failed. Check connection.",
      );
    } finally {
      setLoading(false);
    }
    ...
    >
      <div className="bg-yellow-500 border-2 border-yellow-500 rounded-[2.5rem] shadow-2xl p-8 w-full max-w-md text-slate-900">
        <div className="text-center mb-6">
          ...
          <p className="text-xs font-bold opacity-80 mt-1 text-black">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-800 ml-1">
              Username
            </label>
            <input
              type="text"
          ...
          >
            <ShieldCheck size={16} />{" "}
            {loading ? "Verifying..." : "Sign In System"}
          </button>
        </form>
