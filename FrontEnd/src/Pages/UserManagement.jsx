...
export default function UserManagement() {
  ...
  const themeFont = '"Inter", "Open Sans", "Segoe UI", -apple-system, sans-serif';
    ...
    <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 p-4 pb-20" style={{ fontFamily: themeFont }}>
      ...
      <header className="mb-10 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-widest opacity-70">Security Protocol & Staff Registration</p>
        </div>
      </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-2 relative">
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Assign System Role</label>
                    <div 
                    ...
                    <AnimatePresence>
                        {roleDropdownOpen && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-[105%] left-0 w-full bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 py-2">
                                <div onClick={() => {setFormData({...formData, role: 'user'}); setRoleDropdownOpen(false);}} className="px-5 py-3 text-xs font-bold text-slate-600 hover:bg-yellow-400 hover:text-slate-900 cursor-pointer transition-colors">Standard User (Requests Only)</div>
                                <div onClick={() => {setFormData({...formData, role: 'admin'}); setRoleDropdownOpen(false);}} className="px-5 py-3 text-xs font-bold text-slate-600 hover:bg-yellow-400 hover:text-slate-900 cursor-pointer transition-colors">System Admin (Full Access)</div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
                ...
                <div className="flex flex-col gap-2 relative">
                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Allocate Department</label>
                    <div 
                    ...
                    >
                        ...
                        <span className={`text-xs font-bold flex-1 ${formData.department ? "text-slate-700" : "text-slate-400"}`}>
                    ...
                    <AnimatePresence>
                        {deptDropdownOpen && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-[105%] left-0 w-full bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 py-2 max-h-48 overflow-y-auto">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl h-full flex flex-col justify-center">
            <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 bg-yellow-400 rounded-xl flex items-center justify-center text-slate-900 shadow-lg"><ShieldAlert size={20}/></div>
                <h4 className="text-white font-black text-lg uppercase tracking-tighter">Access Guide</h4>
            </div>
            
            <div className="space-y-6">
              <div className="border-l-4 border-yellow-400 pl-4">
                <p className="text-xs font-black text-white uppercase tracking-widest">Admin Role</p>
                <p className="text-[10px] text-slate-400 mt-1 font-bold leading-relaxed">Full control: Fleet management, financial reports, and user security control.</p>
              </div>
              <div className="border-l-4 border-slate-600 pl-4">
                <p className="text-xs font-black text-white uppercase tracking-widest">Staff Role</p>
                <p className="text-[10px] text-slate-400 mt-1 font-bold leading-relaxed">Standard access: Restricted to creating and tracking their own journey requests.</p>
              </div>
              <div className="border-l-4 border-emerald-500 pl-4">
                <p className="text-xs font-black text-white uppercase tracking-widest">Dept. Locking</p>
                <p className="text-[10px] text-slate-400 mt-1 font-bold leading-relaxed">Users are hard-coded to their department to prevent cross-dept billing errors.</p>
              </div>
            </div>
            
            <div className="mt-10 pt-6 border-t border-slate-800 text-center">
                <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">TMS Security Protocol v2.0</p>
            </div>
          </div>
        </div>
