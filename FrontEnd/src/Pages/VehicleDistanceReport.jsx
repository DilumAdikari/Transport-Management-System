...
import { 
  Truck, Activity, Loader2, FileSearch, 
  Navigation, Gauge, Calendar, Download, ChevronRight
} from "lucide-react";
...
export default function VehicleDistanceReport() {
  ...
  const exportExcel = () => {
    ...
    const exportItems = reportData.map(t => ({
      ...
      "Distance (KM)": (t.endMeter && t.startMeter) ? (t.endMeter - t.startMeter) : 0
    }));
  ...
  const totalDistance = reportData.reduce((acc, curr) => acc + ((curr.endMeter - curr.startMeter) || 0), 0);
      ...
      <div className="p-4 space-y-6 animate-in fade-in duration-700" style={{ fontFamily: themeFont }}>
        ...
        <header className="flex flex-col md:flex-row justify-between items-center bg-white p-5 rounded-3xl border border-slate-100 shadow-sm gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800 tracking-tight">Vehicle Distance Report</h1>
              <p className="text-slate-500 font-medium italic text-[10px]">Odometer-based trip analysis.</p>
            </div>
          </div>
          
          {reportData.length > 0 && (
            <button onClick={exportExcel} className="bg-emerald-600 text-white h-11 px-6 rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-md hover:bg-emerald-700 transition-all flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-5 rounded-[2rem] shadow-sm border border-slate-50 items-end">
          <div className="flex flex-col gap-1.5">
            ...
            <Autocomplete
              ...
              onChange={(e, val) => setSelection({...selection, plate: val || ""})}
              renderInput={(params) => <TextField {...params} placeholder="Plate No..." />}
            />
          </div>
        ...
        {reportData.length > 0 && (
          <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="bg-slate-900 px-8 py-5 rounded-[1.5rem] text-white flex justify-between items-center shadow-lg border border-slate-800">
                <div>
                    <p className="text-yellow-400 text-[9px] font-bold uppercase tracking-[0.15em] mb-0.5">Cumulative Distance</p>
                    <h2 className="text-2xl font-bold tracking-tight">
                      {totalDistance.toLocaleString()} 
                      <span className="text-xs font-medium text-slate-500 ml-1.5 uppercase">km</span>
                    </h2>
                </div>
                <div className="text-right">
                    <p className="text-slate-500 text-[9px] font-bold uppercase tracking-[0.15em] mb-0.5">Trip Total</p>
                    <h2 className="text-xl font-bold text-white">{reportData.length} Journeys</h2>
                </div>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead className="bg-slate-50 border-b border-slate-100 text-[10px] font-semibold uppercase text-slate-400 tracking-wider">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-right text-indigo-600">Travelled</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {reportData.map((t, i) => (
                      <tr key={i} className="hover:bg-slate-50/50 transition-all group">
                        <td className="px-6 py-4 text-sm font-medium text-slate-600">{dayjs(t.date).format('DD/MM/YYYY')}</td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                              <span className="text-sm font-semibold text-slate-800">{t.fromLocation} → {t.toLocation}</span>
                              <span className="text-[10px] text-slate-400 italic font-medium">Driver: {t.driver}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-center text-xs font-mono text-slate-400">{t.startMeter || 0}</td>
                        <td className="px-6 py-4 text-center text-xs font-mono text-slate-400">{t.endMeter || 0}</td>
